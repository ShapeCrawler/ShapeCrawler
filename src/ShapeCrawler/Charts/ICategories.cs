...
internal sealed class Categories : IReadOnlyList<ICategory>
{
    ...
    private List<ICategory> CategoryList()
    {
        ...
        else
        {
            ...
            var sheetName = Regex.Match(normalizedFormula, @".+(?=\!)",RegexOptions.None, TimeSpan.FromMicroseconds(1000)).Value; // eg: Sheet1!A2:A5 -> Sheet1
            var cellsRange = Regex.Match(normalizedFormula, @"(?<=\!).+",RegexOptions.None, TimeSpan.FromMicroseconds(1000)).Value; // eg: Sheet1!A2:A5 -> A2:A5
